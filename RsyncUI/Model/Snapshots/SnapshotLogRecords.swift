...
final class SnapshotRecords {
                                        ...
                                        _ logrecords: [LogRecords])
    {
        ...
        for i in 0 ..< (localrecords[0].records?.count ?? 0) {
            var datestring: String?
            var date: Date?
            if let stringdate = localrecords[0].records?[i].dateExecuted {
            ...
            let record =
                SnapshotLogRecords(
                    ...
                    date: date ?? Date(),
                    resultExecuted: localrecords[0].records?[i].resultExecuted ?? ""
                )
            data.append(record)
        }
